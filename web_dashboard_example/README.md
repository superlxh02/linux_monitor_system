...
- `GET /api/overview` → `QueryLatestScore`
- `GET /api/performance/:serverName?hours=1` → `QueryPerformance`
- `GET /api/rank?order=DESC&page=1&page_size=20` → `QueryScoreRank`
...
- `GET /api/trend/:serverName?hours=1&interval_seconds=300` → `QueryTrend`
