...
void MemoryMonitor::update(monitor::proto::MonitorInfo *monitor_info) {
  // 读取/proc/meminfo文件
  ReadFile file("/proc/meminfo");
  struct mem_info mem_info;
  std::vector<std::string> mem_datas;

  // 解析文件内容并存储到memInfo结构体中
  while (file.read_line(&mem_datas)) {
  ...
  // 将memInfo结构体中的数据存储到monitor_info的mem_info字段中
